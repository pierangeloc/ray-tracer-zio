...
object Renderer {
  ...
  def render(camera: Camera, world: World): ZStreamChunk[PhongReflection with SpatialEntityOperations with RayOperations with AffineTransformationOps, RayTracerError, (Int, Int, Color)] =
    pixelsChunkedStream(camera).mapM { case (px, py) =>
      for {
