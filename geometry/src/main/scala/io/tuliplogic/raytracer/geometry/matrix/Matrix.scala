...
import io.tuliplogic.raytracer.commons.errors.AlgebraicError.{IndexExceedMatrixDimension, MatrixConstructionError}
import mouse.all._
import zio.{IO, _}
    ...
    def fromRows(m: Int, n: Int, rows: L[L[Double]]): ZIO[Any, MatrixConstructionError, Matrix[L]] =
      for {
        _ <- if (L.length(rows) == m && L.toArray(rows).forall(r => L.length(r) == n)) IO.unit
          ...
          IO.fail(
            MatrixConstructionError(s"can't build a matrix out of these rows as dimensions are not correct: m = $m, n = $n, rows.length = ${L.length(rows)}"))
      } yield
