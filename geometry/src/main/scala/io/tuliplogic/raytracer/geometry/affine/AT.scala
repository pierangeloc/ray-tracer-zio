...
object aTModule {
  trait Service[R] {
    def applyTf(tf: AT, vec: Vec): URIO[R, Vec]
    def applyTf(tf: AT, pt: Pt): URIO[R, Pt]
    def compose(first: AT, second: AT): URIO[R, AT]
    def invert(tf: AT): URIO[R, AT]
    def transpose(tf: AT): URIO[R, AT]
    def invertible(
  ...
  type ATModule = Has[Service[Any]]

  object > extends Service[ATModule] {
    override def applyTf(tf: AT, vec: Vec): ZIO[ATModule, Nothing, Vec] =
      ZIO.accessM(_.get.applyTf(tf, vec))
    override def applyTf(tf: AT, pt: Pt): ZIO[ATModule, Nothing, Pt] =
      ZIO.accessM(_.get.applyTf(tf, pt))
    override def compose(first: AT, second: AT): ZIO[ATModule, Nothing, AT] =
      ZIO.accessM(_.get.compose(first, second))
    override def invert(tf: AT): ZIO[ATModule, Nothing, AT] =
