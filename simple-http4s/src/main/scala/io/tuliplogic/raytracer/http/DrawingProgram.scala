...
object DrawingProgram {
  ...
  def draw(sceneBundle: SceneBundle): ZIO[CanvasSerializer with RasteringModule with ATModule with Console with Clock, Nothing, (String, Array[Byte])] =
    for {
      _ <- zio.console.putStrLn("Created scene bundle, now drawing the world...")
      canvas <- RaytracingProgram
        .drawOnCanvas(
          sceneBundle.world,
